...
public class TierOneTests {
  ...
  public void tileConstructiontest_5() {
    ...
    for (int i = 0; i < nodeCount; i++) {
      ...
      assertEquals(tiles.get(tiles.size() - 1), start.getPrev(),
          "start.prev does not link to the last tile after adding tile no: " + i);
      assertEquals(tiles.get(1), start.getNext(),
          "start.next does not link to tile no 1 after adding tile no: " + i);
  ...
  class DummyPlayer extends HumanPlayer {
    ...
    public DummyPlayer(Tile currentTile, String aname) {
      super(currentTile, aname, new ConsoleUi());
    }
  }
