...
public class Player {
  ...
  private List<CardObserver> eventObservers;

  public Player() {
    hand = new LinkedList<Card.Mutable>();
    eventObservers = new LinkedList<CardObserver>();
  }
  ...
  /**
   * Notify when en event happens.
   */
  public void notifyEvent() {
    for (CardObserver o : eventObservers) {
      o.update();
    }
  }

  public void addObs(CardObserver anObserver) {
    eventObservers.add(anObserver);
  }

  public void deleteObs(CardObserver anObserver) {
    eventObservers.remove(anObserver);
  }

  public void giveCard(Player player, Card.Mutable card, boolean cardVisibility) {
    card.show(cardVisibility);
